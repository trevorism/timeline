...
<template>
  ...
  <div class="row">
    ...
    <svg width="1400" height="400">
      <rect v-for="item in timelineData.items" :x="65 + item.beginPixel" y="10" :width="item.endPixel - item.beginPixel" height="40" :fill="item.color" stroke="grey" stroke-width="1"/>
      ...
      <g v-for="(item, index) in timelineData.items">
        <g v-if="item.employer != 'GAP'">
          <rect x="400" :y="130 + (index * 30)" width="20" height="20" :fill="item.color" stroke="black" stroke-width="1"/>
          <text x="440" :y="145 + (index * 30)">{{item.employer}}&nbsp;&nbsp;&nbsp;&nbsp;{{item.beginMonthYear}} - {{item.endMonthYear}}</text>
        </g>
      </g>
    </svg>
